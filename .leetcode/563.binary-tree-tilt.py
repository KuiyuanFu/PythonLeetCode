...
class Solution:
    def findTilt(self, root: Optional[TreeNode]) -> int:
        def recur(p: TreeNode):
            ...
            ln, ls = recur(p.left) if p.left else (0, 0)
            rn, rs = recur(p.right) if p.right else (0, 0)
            n = ln + rn + p.val
            return n, ls + rs + abs(ln - rn)

        return recur(root)[1] if root else 0
        pass
...
if __name__ == '__main__':
    ...
    print('1')
    print('Output :')
    print(str(Solution().findTilt(listToTreeNode([1, 2, 3]))))
    print()

    print('Example 2:')
    print('Input : ')
    print('root = [4,2,9,3,5,null,7]')
    print('Exception :')
    print('15')
    print('Output :')
    print(str(Solution().findTilt(listToTreeNode([4, 2, 9, 3, 5, None, 7]))))
    print()

    print('Example 3:')
    print('Input : ')
    print('root = [21,7,14,1,1,2,2,3,3]')
    print('Exception :')
    print('9')
    print('Output :')
    print(
        str(Solution().findTilt(listToTreeNode([21, 7, 14, 1, 1, 2, 2, 3,
                                                3]))))
    print()
