...
# @lc idea=start
#
# 给定一个有序数组，去重，使其每个元素最多出现两次。
# 直接双指针。从索引为2开始判断，新的元素是否与下一个位置的前两个元素都相等，若相等说明其已经出现两次了，不能再移动了。
#
# @lc idea=end
...
class Solution:
    def removeDuplicates(self, nums: List[int]) -> int:
        ...
        for r in range(2, len(nums)):
            if nums[r] != nums[l - 1] or nums[l - 1] != nums[l - 2]:
                nums[l] = nums[r]
                l += 1
