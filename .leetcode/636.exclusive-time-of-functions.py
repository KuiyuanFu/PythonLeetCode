...
class Solution:
    def exclusiveTime(self, n: int, logs: List[str]) -> List[int]:
        ...
        for log in logs:
            ss = log.split(':')
            fid, start, timestamp = int(ss[0]), ss[1] == 'start', int(ss[2])
            if start:
                s.append([fid, timestamp, 0])
            else:
                _, sts, occupancy = s.pop()
                time = timestamp - sts + 1
                res[fid] += time - occupancy
                s[-1][-1] += time
        return res
...
if __name__ == '__main__':
    print('Example 1:')
    print('Input : ')
    print('n = 2, logs = ["0:start:0","1:start:2","1:end:5","0:end:6"]')
    print('Exception :')
    print('[3,4]')
    print('Output :')
    print(
        str(Solution().exclusiveTime(
    ...
    print(
        str(Solution().exclusiveTime(2, [
            "0:start:0", "0:start:2", "0:end:5", "1:start:7", "1:end:7",
            "0:end:8"
        ])))
    print()
    ...
    print('Example 5:')
    print('Input : ')
    print('n = 1, logs = ["0:start:0","0:end:0"]')
    print('Exception :')
    print('[1]')
    print('Output :')
    print(str(Solution().exclusiveTime(1, ["0:start:0", "0:end:0"])))
    print()
