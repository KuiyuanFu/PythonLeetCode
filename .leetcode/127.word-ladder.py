...
#
# 词梯。给定一组单词，与起始词，结束词。
# 从起始词开始，每一次从给定的单词序列中，找到一个单词，满足只相差一个字符。转换为结束词。
# 求最少的转换次数。
# 实际上，这是一个图问题，求起始词的结点到结束词结点的最短路径。
# Dijkstra 算法即可解。
# 其实由于路径权重都为1，直接广度优先即可。
# 使用Dijkstra竟然超时了。那只能广度优先了。
# 单纯广度优先也超时，额外增加两端计算。
# 也非常慢，根据其他提交，应该是判断两字符串是否一致，花费了大量时间。可以使用令一个字符串，每个位置的所有可能取值，判断是否有这样的取值。
#
# @lc idea=end

# @lc group=breadth-first-search
...
def listToListNode(l: List[str]) -> ListNode:
        ...
        while  beginSet and endSet:
            ...
            if flag:
                ...
                for val in list(beginSet):
                    for tVal in endSet:
                        if isAdjacency(val, tVal):
                            return length

                    pnr = pseudoNotReached
                    while pnr.next:
                        if isAdjacency(val, pnr.next.val):
                            pnrNext = pnr.next
                            pnr.next = pnr.next.next

                            beginSetNew.add(pnrNext.val)
                ...
                endSetNew = set()
                for val in list(endSet):
                    for tVal in beginSet:
                        if isAdjacency(val, tVal):
                            return length
                    ...
                    while pnr.next:
                        if isAdjacency(val, pnr.next.val):
                            ...
                            endSetNew.add(pnrNext.val)

                            if pnrNext.val in beginSet:
                                return length
                        else:
                            pnr = pnr.next
                endSet = endSetNew
