# @lc app=leetcode id=460 lang=python3
#
# [460] LFU Cache
#
# https://leetcode.com/problems/lfu-cache/description/
#
# algorithms
# Hard (37.80%)
# Likes:    2427
# Dislikes: 168
# Total Accepted:    123.4K
# Total Submissions: 326K
# Testcase Example:  '["LFUCache","put","put","get","put","get","get","put","get","get","get"]\n' +
# '[[2],[1,1],[2,2],[1],[3,3],[2],[3],[4,4],[1],[3],[4]]'
#
# Design and implement a data structure for a Least Frequently Used (LFU)
# cache.
#
# Implement the LFUCache class:
#
#
# LFUCache(int capacity) Initializes the object with the capacity of the data
# structure.
# int get(int key) Gets the value of the key if the key exists in the cache.
# Otherwise, returns -1.
# void put(int key, int value) Update the value of the key if present, or
# inserts the key if not already present. When the cache reaches its capacity,
# it should invalidate and remove the least frequently used key before
# inserting a new item. For this problem, when there is a tie (i.e., two or
# more keys with the same frequency), the least recently used key would be
# invalidated.
#
#
# To determine the least frequently used key, a use counter is maintained for
# each key in the cache. The key with the smallest use counter is the least
# frequently used key.
#
# When a key is first inserted into the cache, its use counter is set to 1 (due
# to the put operation). The use counter for a key in the cache is incremented
# either a get or put operation is called on it.
#
# The functions get and put must each run in O(1) average time complexity.
#
#
# Example 1:
#
#
# Input
# ["LFUCache", "put", "put", "get", "put", "get", "get", "put", "get", "get",
# "get"]
# [[2], [1, 1], [2, 2], [1], [3, 3], [2], [3], [4, 4], [1], [3], [4]]
# Output
# [null, null, null, 1, null, -1, 3, null, -1, 3, 4]
#
# Explanation
# // cnt(x) = the use counter for key x
# // cache=[] will show the last used order for tiebreakers (leftmost element
# is  most recent)
# LFUCache lfu = new LFUCache(2);
# lfu.put(1, 1);   // cache=[1,_], cnt(1)=1
# lfu.put(2, 2);   // cache=[2,1], cnt(2)=1, cnt(1)=1
# lfu.get(1);      // return 1
# ⁠                // cache=[1,2], cnt(2)=1, cnt(1)=2
# lfu.put(3, 3);   // 2 is the LFU key because cnt(2)=1 is the smallest,
# invalidate 2.
# // cache=[3,1], cnt(3)=1, cnt(1)=2
# lfu.get(2);      // return -1 (not found)
# lfu.get(3);      // return 3
# ⁠                // cache=[3,1], cnt(3)=2, cnt(1)=2
# lfu.put(4, 4);   // Both 1 and 3 have the same cnt, but 1 is LRU, invalidate
# 1.
# ⁠                // cache=[4,3], cnt(4)=1, cnt(3)=2
# lfu.get(1);      // return -1 (not found)
# lfu.get(3);      // return 3
# ⁠                // cache=[3,4], cnt(4)=1, cnt(3)=3
# lfu.get(4);      // return 4
# ⁠                // cache=[3,4], cnt(4)=2, cnt(3)=3
#
#
#
# Constraints:
#
#
# 0 <= capacity <= 10^4
# 0 <= key <= 10^5
# 0 <= value <= 10^9
# At most 2 * 10^5 calls will be made to get and put.
#
#
#
#
#

# @lc tags=design

# @lc imports=start
from sys import prefix
from imports import *

# @lc imports=end

# @lc idea=start
#
# 最不频繁使用替换的缓存。
# 使用双向链表表示同样次数的key，之后一个字典指向不同的链表，另一个字典指向不同的key。
#
# @lc idea=end

# @lc group=

# @lc rank=

# @lc code=start


class Node:
    def __init__(self, fre=0, key=0, value=0, pre=None, next=None) -> None:
        self.pre = pre if pre else self
        self.next = next if next else self
        self.fre = fre
        self.key = key
        self.value = value
        pass

    def remove(self):
        pn = self.pre
        nn = self.next
        pn.next = nn
        nn.pre = pn

    def addPre(self, node):
        pn = self.pre
        self.pre = node
        pn.next = node
        node.next = self
        node.pre = pn

    def removeNext(self):
        nn = self.next
        self.next = nn.next
        self.next.pre = self
        return nn

    def empty(self):
        return self == self.next


class LFUCache:
    def __init__(self, capacity: int):
        self.capacity = capacity
        self.count = 0
        self.minFre = 1
        self.nodes = {}
        self.fres = defaultdict(lambda: Node())

    def get(self, key: int) -> int:
        if key not in self.nodes:
            return -1
        node = self.nodes[key]
        self.moveToNextFre(node)
        return node.value

    def moveToNextFre(self, node):
        node.remove()
        if self.fres[self.minFre].empty():
            self.minFre += 1
        node.fre += 1
        self.fres[node.fre].addPre(node)

    def _pop(self):
        if self.count < self.capacity:
            return True
        d = self.fres[self.minFre]
        if d.empty():
            return False
        n = d.removeNext()
        self.nodes.pop(n.key)
        self.count -= 1
        return True

    def put(self, key: int, value: int) -> None:
        if key in self.nodes:
            node = self.nodes[key]
            node.value = value
            self.moveToNextFre(node)

        elif self._pop():
            node = Node(key=key, value=value, fre=1)
            self.nodes[key] = node
            self.fres[1].addPre(node)
            self.minFre = min(self.minFre, 1)
            self.count += 1


# @lc code=end

# @lc main=start
if __name__ == '__main__':
    o = LFUCache(2)
    o.put(1, 1)
    o.put(2, 2)
    print(o.get(1))
    o.put(3, 3)
    print(o.get(2))
    print(o.get(3))
    o.put(4, 4)
    print(o.get(1))
    print(o.get(3))
    print(o.get(4))
    pass
# @lc main=end