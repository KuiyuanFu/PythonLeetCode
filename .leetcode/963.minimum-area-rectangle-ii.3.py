...
class Solution:
    ...
    def minAreaFreeRect(self, points: List[List[int]]) -> float:
        ...
        def unique(i, j, k, l):
            return i != k and i != l and j != k and j != l

        diagonals = defaultdict(list)
        for i, j in combinations(points, 2):
            diagonals[getDiagonal(i, j)].append((i, j))

        for values in diagonals.values():
            for (i, j), (k, l) in combinations(values, 2):
                if unique(i, j, k, l) and isRightAngle(k, i, l):
                    res = min(res, disSquare(i, l)**0.5 * disSquare(i, k)**0.5)
