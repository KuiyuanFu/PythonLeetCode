...
# @lc idea=start
#
# 给定一个二维矩阵board，其中有障碍，只能向右或下走的条件下，从左上到右下有多少种路径。
# 动态规划，先初始化最后一行和最右一列，根据是否有障碍来确定是否可以同行，之后反向动态规划。
#
# @lc idea=end
            ...
            for j in range(len(obstacleGrid[0]) - 2, -1, -1):
                if obstacleGrid[i][j] == 0:
                    obstacleGrid[i][j] += obstacleGrid[
                        i + 1][j] if obstacleGrid[i + 1][j] < 0 else 0
                    obstacleGrid[i][j] += obstacleGrid[i][
                        j + 1] if obstacleGrid[i][j + 1] < 0 else 0
...
if __name__ == '__main__':
    ...
    print('Output :')
    print(
        str(Solution().uniquePathsWithObstacles([[0, 0, 0], [0, 1, 0],
                                                 [0, 0, 0]])))
    print('Exception :')
    print('2')
    print()
