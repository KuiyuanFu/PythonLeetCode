...
class Solution:
    def shoppingOffers(self, price: List[int], special: List[List[int]],
        ...
        buffer = {}
        length = len(needs)
        lr = list(range(length))

        def valid(needs, spec):
            for i in lr:
                if needs[i] < spec[i]:
                    return False
            return True

        def recur(needs: List[int]):
            k = tuple(needs)
            if k in buffer:
                return buffer[k]
            res = sum((price[i] * needs[i]) for i in lr)
            for s in special:
                if valid(needs, s):
                    needsn = [(needs[i] - s[i]) for i in lr]
                    c = s[-1] + recur(needsn)
                    if c < res:
                        res = c
            buffer[k] = res
...
if __name__ == '__main__':
    print('Example 1:')
    print('Input : ')
    print('price = [2,5], special = [[3,0,5],[1,2,10]], needs = [3,2]')
    print('Exception :')
    print('14')
    print('Output :')
    print(
        str(Solution().shoppingOffers([2, 5], [[3, 0, 5], [1, 2, 10]],
                                      [3, 2])))
    print()

    print('Example 2:')
    print('Input : ')
    print('price = [2,3,4], special = [[1,1,0,4],[2,2,1,9]], needs = [1,2,1]')
    print('Exception :')
    print('11')
    print('Output :')
    print(
        str(Solution().shoppingOffers([2, 3, 4], [[1, 1, 0, 4], [2, 2, 1, 9]],
                                      [1, 2, 1])))
    print()
