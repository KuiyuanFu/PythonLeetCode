...
# @lc idea=start
#
# 跳跃，每个元素按照顺序有奇偶之分，处在奇位置上，可以跳跃到小于等于其的最大元素上，如果有多个最大元素，则跳到索引最小的上；处在偶位置上，可以条约到大于等于其的最小元素，如果有多个最小元素，则跳到索引最小的上；
# 求有多少个索引位置，可以到达数组末尾。
# 有向图，每个节点只有一个后续节点，将图反转，求从结尾可以到达多少个节点。
#
# @lc idea=end
...
class Solution:
    ...
    def oddEvenJumps(self, arr: List[int]) -> int:
        ...
        # sort by idx
        smallests = defaultdict(list)
        for i, a in enumerate(arr):
            smallests[a].append(i)
        nums = list(smallests.keys())
        nums.sort()

        # num relation
        numList = {}
        for i, n in enumerate(nums):
            pre = None if i == 0 else nums[i - 1]
            next = None if i == len(nums) - 1 else nums[i + 1]
            numList[n] = [smallests[n], 0, pre, next]

        # edge
        oddNeighborss = [[] for _ in range(length)]
        evenNeighborss = [[] for _ in range(length)]
        for i, n in enumerate(arr):
            nl = numList[n]
            nl[1] += 1
            idices, idxInIdices, pre, next = nl

            oddl = nl
            evenl = nl
            if idxInIdices == len(idices):
                if next:
                    numList[next][2] = pre
                if pre:
                    numList[pre][3] = next
                oddl = numList[next] if next else None
                evenl = numList[pre] if pre else None
            if oddl:
                srcIdx = oddl[0][oddl[1]]
                oddNeighborss[srcIdx].append(i)
            if evenl:
                srcIdx = evenl[0][evenl[1]]
                evenNeighborss[srcIdx].append(i)

        q = [(length - 1, True), (length - 1, False)]
        oddVisited = set([length - 1])
        evenVisited = set([length - 1])
        while q:
            idx, isOdd = q.pop()
            neighborss, visited = (oddNeighborss,
                                   oddVisited) if isOdd else (evenNeighborss,
                                                              evenVisited)

            neighbors = neighborss[idx]
            for neighbor in neighbors:
                if neighbor not in visited:
                    visited.add(neighbor)
                    q.append((neighbor, not isOdd))

        return len(oddVisited)
        pass
...
if __name__ == '__main__':
    print('Example 1:')
    print('Input : ')
    print('arr = [10,13,12,14,15]')
    print('Exception :')
    print('2')
    print('Output :')
    print(str(Solution().oddEvenJumps([10, 13, 12, 14, 15])))
    print()
    ...
    print('3')
    print('Output :')
    print(str(Solution().oddEvenJumps([2, 3, 1, 1, 4])))
    print()
    ...
    print('3')
    print('Output :')
    print(str(Solution().oddEvenJumps([5, 1, 3, 4, 2])))
    print()
